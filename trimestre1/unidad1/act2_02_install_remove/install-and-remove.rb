      ...
      case campos[1]
	    when 'i','install'
	      if paquete == ""
		    ...
		    puts "El paquete #{campos[0]} ya se encuentra instalado. No se instalará."
	      end
	    when 'r','remove'
	      if paquete == ""
		    puts "El paquete #{campos[0]} no se puede eliminar ya que no se encuentra instalado"
	      else
		    puts "Desinstalando #{campos[0]}"
		    `apt-get purge #{campos[0]} -y`
		    puts "#{campos[0]} desinstalado"
  ...
  when "opensuse"
    filas.each do |fila|
      ...
      case campos[1]
  	    when 'i','install'
	      if paquete != ""
	        puts "El paquete #{campos[0]} no se puede instalar, ya está instalado."
	      else
			puts "Instalando #{campos[0]}"
